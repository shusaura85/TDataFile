...
type
  ...
  *)
  TBCPHPrintFlag = (pfSelectionOnly, pfSelectionBold, pfMonochrome,
    pfUseBackgroundColor, pfCurrentViewOnly, pfIncludeRuler);
  // @exclude()
  TBCHPrintFlags = set of TBCPHPrintFlag;

  // @exclude(print header/footer)
  TBCHPrintHeaders = array[0..1] of string;

  (* this event is called when @link(PropertiesAsString) is read or written. TBCHexEditorEx
  ...
  *)
  ...
  TBCHQueryPublicPropertyEvent = procedure(Sender: TObject; const PropertyName:
    string;
    var IsPublic: boolean) of object;

  // enhanced hex editor
  TBCHexEditorEx = class(TCustomBCHexEditor)
  private
    { Private-Deklarationen }
    FCreateBackups: boolean;
    FBackupFileExt: string;
    FOleDragDrop: boolean;
    FDropTarget: TBCHDropTarget;
    FOleFormat: array[TBCHOLEOperation] of TClipFormat;
    FOleDragging, FOleStartDrag: boolean;
    FOleDragX, FOleDragY: integer;
    FOleWasTarget: boolean;
    FPrintOptions: TBCHPrintOptions;
    (*FDoChange: boolean;
    FHasChanged: boolean;*)
    ...
    FOnQueryPublicProperty: TBCHQueryPublicPropertyEvent;
    ...
    procedure SetPrintOptions(const Value: TBCHPrintOptions);
  ...
  published
    ...
    *)
    ...
    // do we support other formats than CF_BCHEXEDITOR and CF_HDROP?
    property SupportsOtherClipFormats: boolean read FSupportsOtherClipFormats
      write FSupportsOtherClipFormats default True;
    // print/preview options, see @link(TBCHPrintOptions)
    property PrintOptions: TBCHPrintOptions read FPrintOptions write
      SetPrintOptions;
    ...
    *)
    ...
    (* this event is called when @link(PropertiesAsString) is read or written.
       (see @link(TBCHQueryPublicPropertyEvent))
    *)
    property OnQueryPublicProperty: TBCHQueryPublicPropertyEvent read
  ...
  TBCHDropTarget = class(TInterfacedObject, IDropTarget)
  private
    FEditor: TBCHexEditorEx;
    FEditorHandle: THandle;
    FActive: boolean;
    procedure SetActive(const Value: boolean);
  public
    constructor Create(Editor: TBCHexEditorEx);
  ...
  TBCHPrintOptions = class(TPersistent)
  private
    FMargins: TRect;
    FHeaders: TBCHPrintHeaders;
    FFlags: TBCHPrintFlags;
  ...
  published
    ...
    *)
    property Flags: TBCHPrintFlags read FFlags write FFlags;
  end;

procedure BCHSetHexClipboardData(Data: Pointer; DataSize: integer; ScrapFileName:
      string; TextAsHex, SwapNibbles: boolean);

  // default print margins
const
  BCH_DEF_PRINT_MARGINS: TRect = (Left: 20; Top: 15; Right: 25; Bottom: 25);
...
type
  ...
  TBCHEnumFormatETC = class(TInterfacedObject, IEnumFormatETC)
  private
  ...
  // ole drop source
  TBCHDropSource = class(TInterfacedObject, IDropSource)
  public
  ...
  // ole data container
  TBCHDataObject = class(TInterfacedObject, IDataObject)
  private
    FData: Pointer;
    FDataSize: integer;
    FFileName: string;
  ...
  // draw hex on canvas
  TBCHCanvasPrinter = class(TObject)
  private
    FMargins: TRect;
    FHeaders,
      FPrintHeaders: TBCHPrintHeaders;
  ...
  public
    constructor Create(AEditor: TBCHexEditorEx; ACanvas: TCanvas; AFlags:
      TBCHPrintFlags; AMargins: TRect; AHeaders: TBCHPrintHeaders);
...
constructor TBCHexEditorEx.Create(AOwner: TComponent);
begin
  inherited;
  //FHasChanged := False;
  //FDoChange := True;
  ...
  FPrintOptions := TBCHPrintOptions.Create;
  FPrintFont := TFont.Create;
  FPrintFont.Assign(Font);
  FUseEditorFontForPrinting := True;
  FOffsetPopupMenu := nil;
  if not (csDesigning in ComponentState) then
    FDropTarget := TBCHDropTarget.Create(self); // not in delphi ide
end;

// destructor

destructor TBCHexEditorEx.Destroy;
begin
...
function TBCHexEditorEx.CanCopy: boolean;
begin
...
function TBCHexEditorEx.CanCut: boolean;
begin
...
function TBCHexEditorEx.CanPaste: boolean;
var
  LifData: IDataObject;
  LIntEffect: integer;
begin
...
function TBCHexEditorEx.CBCopy: boolean;
begin
  Result := CanCopy;
  if Result then
  begin
    WaitCursor;
    try
      FClipData := TBCHDataObject.CreateFromStream(DataStorage, Min(SelStart,
...
function TBCHexEditorEx.CBCut: boolean;
begin
...
function TBCHexEditorEx.CBPaste: boolean;
var
  LifData: IDataObject;
  LIntEffect: integer;
begin
...
procedure TBCHexEditorEx.CreateRangeUndo(const aStart, aCount: integer;
  sDesc: string);
var
  bMod: boolean;
begin
...
function TBCHexEditorEx.BeginUpdate: integer;
begin
  Inc(FPaintUpdateCounter);
  Result := FPaintUpdateCounter;
end;

function TBCHexEditorEx.EndUpdate: integer;
begin
...
function TBCHexEditorEx.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint):
  boolean;
begin
...
function TBCHexEditorEx.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint):
  boolean;
begin
...
procedure TBCHexEditorEx.KeyDown(var Key: word; Shift: TShiftState);
begin
  inherited;
  case Key of
    // CTRL+A: select all
...
procedure TBCHexEditorEx.PrepareOverwriteDiskFile;
...
begin
  ...
  begin
    ...
    if FileExists(LStrBackup) and not DeleteFile(LStrBackup) then
      raise EBCHexEditor.CreateFmt(ERR_BACKUP_DELETE,
        [LStrBackup, SysErrorMessage(GetLastError)]);

    if (not MoveFile(PChar(FileName), PChar(LStrBackup))) then
      raise EBCHexEditor.CreateFmt(ERR_BACKUP_CREATE,
        [LStrBackup, SysErrorMessage(GetLastError)]);
  end;
end;
...
procedure TBCHexEditorEx.Save;
begin
  if not HasFile then
    raise EBCHexEditor.Create(ERR_NOFILE);
  SaveToFile(FileName);
end;

// prepare ole dragging

procedure TBCHexEditorEx.MouseDown(Button: TMouseButton; Shift: TShiftState; X,
  Y: integer);
begin
  inherited;
...
procedure TBCHexEditorEx.MouseMove(Shift: TShiftState; X, Y: integer);
var
  LHrsOperation: HRESULT;
  LIntEffect: integer;
  LobjData: TBCHDataObject;
begin
  inherited;
  ...
  begin
    ...
    // start ole dragging
    try
      LobjData := TBCHDataObject.CreateFromStream(DataStorage, Min(SelStart,
        SelEnd), SelCount, ExtractFileName(FileName), FClipboardAsHexText,
        SwapNibbles);
      if not ReadOnlyView then
        LHrsOperation := DoDragDrop(LobjData, TBCHDropSource.Create,
...
procedure TBCHexEditorEx.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y:
  integer);
begin
...
procedure TBCHexEditorEx.SetOleDragDrop(const Value: boolean);
begin
...
procedure TBCHexEditorEx.CreateWnd;
begin
  inherited;
  if not (csDesigning in ComponentState) then
  begin
...
function TBCHexEditorEx.InsertOLEData(const dataObj: IDataObject; const
...
begin
  ...
  try
    // haupt-format?
    if ((FOLEFormat[Operation] = CF_BCHEXEDITOR) or (FOLEFormat[Operation] =
...
function TBCHexEditorEx.SupportsOLEData(const dataObj: IDataObject; const
...
function TBCHexEditorEx.OLEHasSupportedFormat(const dataObj: IDataObject; const
...
function TBCHexEditorEx.ModifyOLEDropEffect(const grfKeyState: integer; const
...
function TBCHexEditorEx.GetMyOLEFormats: TClipFormats;
begin
  if FSupportsOtherClipFormats then
    SetLength(Result, 17)
  else
    SetLength(Result, 2);
  Result[0] := CF_BCHEXEDITOR;
  Result[1] := CF_HDROP;
  if FSupportsOtherClipFormats then
  begin
    Result[2] := CF_TEXT;
...
procedure TBCHexEditorEx.WMDestroy(var Message: TWMDestroy);
begin
  inherited;
  if ComponentState * [csLoading, csDesigning] = [] then
...
function TBCHexEditorEx.DumpUndoStorage(const FileName: string): boolean;
begin
  Result := False;
...
procedure TBCHexEditorEx.SetPrintOptions(const Value: TBCHPrintOptions);
begin
...
function TBCHexEditorEx.PrintToCanvas(ACanvas: TCanvas; const APage: integer;
  const AMargins: TRect): integer;
var
  LObjPrinter: TBCHCanvasPrinter;
  LSetFlags: TBCHPrintFlags;
begin
  if APage < 0 then
    raise EBCHexEditor.Create(ERR_INVALID_PAGE);
  WaitCursor;
  LSetFlags := FPrintOptions.Flags;
  try
    if SelCount = 0 then
      Exclude(LSetFlags, pfSelectionOnly);
    LObjPrinter := TBCHCanvasPrinter.Create(self, ACanvas, LSetFlags, AMargins,
    ...
    try
      ...
      if APage > 0 then
        if LObjPrinter.DrawOrCalc(False, APage) < 1 then
          raise EBCHexEditor.Create(ERR_PRINTING_FAILED);
    finally
      LObjPrinter.Free;
    end;
  finally
...
function TBCHexEditorEx.PrintPreview(const Page: integer): TMetaFile;
var
...
procedure TBCHexEditorEx.Print(const Page: integer);
var
  LmtfTemp: TMetaFile;
begin
  if Page < 1 then
...
function TBCHexEditorEx.PrinterMarginRect: TRect;
var
...
function TBCHexEditorEx.PrintNumPages: integer;
begin
  PrintPreview(0).Free;
  Result := FPrintPages;
end;
...
procedure TBCHexEditorEx.ReleaseClipboard(const Flush: boolean);
begin
...
function TBCHexEditorEx.OwnsClipBoard: boolean;
begin
  Result := OleIsCurrentClipBoard(FClipData) = S_OK;
end;

procedure TBCHexEditorEx.SetPrintFont(const Value: TFont);
begin
  FPrintFont.Assign(Value);
  FUseEditorFontForPrinting := False;
end;

procedure TBCHexEditorEx.DoContextPopup(MousePos: TPoint; var Handled: boolean);
begin
  inherited;
...
procedure TBCHexEditorEx.SetOffsetPopupMenu(const Value: TPopupMenu);
begin
...
function TBCHexEditorEx.GetOffsetPopupMenu: TPopupMenu;
begin
  Result := FOffsetPopupMenu;
end;

procedure TBCHexEditorEx.Notification(AComponent: TComponent; Operation:
  TOperation);
begin
  inherited;
...
function TBCHexEditorEx.CanCreateUndo(const aKind: TBCHUndoFlag;
...
(*procedure TBCHexEditorEx.Changed;
begin
...
function TBCHexEditorEx.GetBookmarksAsString: string;

  procedure AddBM(const bm: TBCHBookmark);
  begin
...
begin
  Result := '$';
  LBMSet := False;
  for LIntLoop := Low(TBCHBookmarks) to High(TBCHBookmarks) do
  begin
...
function TBCHexEditorEx.ExecuteAction(Action: TBasicAction): Boolean;
begin
...
function TBCHexEditorEx.UpdateAction(Action: TBasicAction): Boolean;
begin
  Result := Focused;
  if Result then
  begin
...
procedure TBCHexEditorEx.SetBookMarksAsString(Value: string);
var
  LIntLoop, LIntCheck, LIntCheck1, LIntPos: integer;
  LBoolChars: boolean;
  LRecBook: TBCHBookmark;
  ...
  begin
    ...
    try
      ...
      raise EBCHexEditor.Create(ERR_INVALID_BOOKFMT);
    end;
  end;
...
const
  BCTH_PUBLIC_PROPS: array[0..68] of string = (
    'ShowRuler',
    'DrawGutter3D',
    'CreateBackup',
    'BackupExtension',
...
function TBCHexEditorEx.IsPropPublic(PropName: string): boolean;
var
  LIntLoop: integer;
begin
  Result := False;
  for LIntLoop := Low(BCTH_PUBLIC_PROPS) to High(BCTH_PUBLIC_PROPS) do
    if AnsiCompareText(PropName, BCTH_PUBLIC_PROPS[LIntLoop]) = 0 then
    begin
      Result := True;
      Break;
    end;
...
function TBCHexEditorEx.GetPropertiesAsString: string;
var
  sl: TStrings;
      ...
      for LIntCount := 0 to Pred(LIntCount) do
        with LPtrProps^[LIntCount]^ do
          if PropType^^.Kind = tkClass then
            ...
            sl.Add(Prefix + string(Name) + '=' +string(GetPropValue(Ref, string(Name), False)));
...
procedure TBCHexEditorEx.SetPropertiesAsString(const Value: string);
var
  LStrData: TStrings;
...
procedure TBCHexEditorEx.Paint;
begin
  //inherited;
...
procedure TBCHexEditorEx.DblClick;
var
  LptMouse: TPoint;
  LIntPos: Integer;
begin
...
procedure TBCHexEditorEx.PasteData(P: Pointer; const ACount: integer;
...
procedure TBCHexEditorEx.BookmarkChanged;
begin
  if not FBookmarksNoChange then
    inherited;
end;

function TBCHexEditorEx.UndoBeginUpdate(const StrUndoDesc: string = ''):
  integer;
begin
...
function TBCHexEditorEx.UndoEndUpdate: integer;
begin
...
procedure TBCHexEditorEx.WriteBuffer(const Buffer; const Index,
  Count: Integer);
begin
  inherited;
  FModified := True;
...
constructor TBCHDropTarget.Create(Editor: TBCHexEditorEx);
begin
  inherited Create;
  FEditor := Editor;
  FEditorHandle := 0;
  FActive := False;
  _AddRef; // don't free automatically because it's an object in TBCHexEditorEx
end;

// tinterfacedobject auto-destructor hook

procedure TBCHDropTarget.BeforeDestruction;
begin
...
function TBCHDropTarget.DragEnter(const dataObj: IDataObject; grfKeyState:
...
function TBCHDropTarget.DragLeave: HResult;
begin
  Result := S_OK;
...
function TBCHDropTarget.DragOver(grfKeyState: integer; pt: TPoint; var dwEffect:
  integer): HResult;
begin
  Result := FEditor.ModifyOLEDropEffect(grfKeyState, pt, dwEffect);
  if Result = S_OK then
...
function TBCHDropTarget.Drop(const dataObj: IDataObject; grfKeyState: integer;
  pt: TPoint; var dwEffect: integer): HResult;
begin
  try
    Result := FEditor.SupportsOLEData(dataObj, grfKeyState, pt, dwEffect,
...
procedure TBCHDropTarget.SetActive(const Value: boolean);
begin
...
constructor TBCHEnumFormatETC.Create;
begin
  inherited Create;
  FIndex := 0;
  with FFormats[0] do
  begin
    cfFormat := CF_BCHEXEDITOR;
    ptd := nil;
...
function TBCHEnumFormatETC.Clone(out Enum: IEnumFormatEtc): HResult;
begin
  Enum := TBCHEnumFormatETC.Create;
  Result := S_OK;
end;

// iterate over all format records

function TBCHEnumFormatETC.Next(celt: integer; out elt; pceltFetched: PLongint):
  HResult;
var
  LIntLoop: integer;
...
function TBCHEnumFormatETC.Reset: HResult;
begin
  FIndex := 0;
  Result := S_OK;
end;
...
function TBCHEnumFormatETC.Skip(celt: integer): HResult;
begin
...
function TBCHDropSource.GiveFeedback(dwEffect: integer): HResult;
begin
...
function TBCHDropSource.QueryContinueDrag(fEscapePressed: BOOL; grfKeyState:
  integer): HResult;
begin
...
constructor TBCHDataObject.Create(Data: Pointer; DataSize: integer;
  ScrapFileName: string; TextAsHex, SwapNibbles: boolean);
begin
  inherited Create;
  FData := nil;
...
constructor TBCHDataObject.CreateFromStream(Stream: TStream; Position, DataSize:
...
procedure TBCHDataObject.BeforeDestruction;
begin
...
function TBCHDataObject.DAdvise(const formatetc: TFormatEtc; advf: integer; const
  advSink: IAdviseSink; out dwConnection: integer): HResult;
begin
  Result := OLE_E_ADVISENOTSUPPORTED;
end;

function TBCHDataObject.DUnadvise(dwConnection: integer): HResult;
begin
  Result := OLE_E_ADVISENOTSUPPORTED;
end;

function TBCHDataObject.EnumDAdvise(out enumAdvise: IEnumStatData): HResult;
begin
  Result := OLE_E_ADVISENOTSUPPORTED;
end;
...
function TBCHDataObject.EnumFormatEtc(dwDirection: integer; out enumFormatEtc:
  IEnumFormatEtc): HResult;
begin
  enumFormatETC := nil;
  if dwDirection = DATADIR_GET then
  begin
    enumFormatETC := TBCHEnumFormatETC.Create;
    Result := S_OK;
  end
  else
    Result := E_NOTIMPL;
...
function TBCHDataObject.GetCanonicalFormatEtc(const formatetc: TFormatEtc; out
...
function TBCHDataObject.GetData(const formatetcIn: TFormatEtc; out medium:
  TStgMedium): HResult;
var
...
begin
  ...
  if Result = S_OK then
  begin
    if formatetcIn.cfFormat = CF_BCHEXEDITOR then
    ...
    begin
      ...
      try
        try
          ...
          else if formatetcIn.cfFormat = CF_BCHEXEDITOR then
          begin
...
function TBCHDataObject.GetDataHere(const formatetc: TFormatEtc;
...
function TBCHDataObject.QueryGetData(const formatetc: TFormatEtc): HResult;
begin
  Result := DV_E_FORMATETC;
  with formatetc do
  begin
    if dwAspect <> DVASPECT_CONTENT then
      ...
      then
      Result := DV_E_LINDEX
    else if (cfFormat = CF_BCHEXEDITOR) or (cfFormat = CF_TEXT) or
...
function TBCHDataObject.SetData(const formatetc: TFormatEtc; var medium:
...
constructor TBCHCanvasPrinter.Create(AEditor: TBCHexEditorEx; ACanvas: TCanvas;
  AFlags: TBCHPrintFlags; AMargins: TRect; AHeaders: TBCHPrintHeaders);
begin
  inherited Create;
  FMargins := AMargins;
  FCanvas := ACanvas;
...
function TBCHCanvasPrinter.BuildHeader(const S: string; const Page: integer):
  string;
var
  LIntLoop: integer;
begin
        ...
        '>':
          begin
            if not FEditor.UnicodeChars then
              Result := Result + BCHTranslationDesc
                [FEditor.Translation]
            else
            begin
              if not FEditor.UnicodeBigEndian then
                Result := Result + BCH_UC
              else
                Result := Result + BCH_UC_BE
            end;
          end;
        '<':
          begin
            if not FEditor.UnicodeChars then
              Result := Result +
                BCHTranslationDescShort
                [FEditor.Translation]
            else
            begin
              if not FEditor.UnicodeBigEndian then
                Result := Result + BCH_UC_S
              else
                Result := Result + BCH_UC_BE_S
            end;
          end
      else
...
procedure TBCHCanvasPrinter.Draw(const Page: integer);
begin
...
function TBCHCanvasPrinter.DrawOrCalc(const JustCalc: boolean; const Page:
  integer): integer;
...
function TBCHCanvasPrinter.GetLinesPerPage: integer;
var
  LIntSize: integer;
  LSetTempFlags: TBCHPrintFlags;
begin
...
constructor TBCHPrintOptions.Create;
begin
  inherited;
  FMargins := BCH_DEF_PRINT_MARGINS;
  FFlags := [pfMonochrome, pfSelectionBold];
end;

// copy props

procedure TBCHPrintOptions.Assign(Source: TPersistent);
begin
  inherited;
  if Source is TBCHPrintOptions then
    with TBCHPrintOptions(Source) do
    begin
...
function TBCHPrintOptions.GetHeader(const Index: integer): string;
begin
...
function TBCHPrintOptions.GetMargin(const Index: integer): integer;
begin
  case Index of
...
procedure TBCHPrintOptions.SetHeader(const Index: integer; const Value:
  string);
begin
...
procedure TBCHPrintOptions.SetMargin(const Index, Value: integer);
begin
  case Index of
...
procedure BCHSetHexClipboardData(Data: Pointer; DataSize: integer; ScrapFileName:
      string; TextAsHex, SwapNibbles: boolean);
begin
  OleCheck(OleSetClipboard(TBCHDataObject.Create(Data, DataSize, ScrapFileName,
...
OleInitialize(nil);
CF_BCHEXEDITOR := RegisterClipboardFormat(PChar(MPTH_CF));
