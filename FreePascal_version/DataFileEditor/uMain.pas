...
type
  ...
  public
    { Public declarations }
    df: TDataFile;
    procedure CreateNodes(str:string; path:string = ''; nodeparent:TTreeNode = nil);
    procedure LoadHive;
...
procedure TfrmMain.CreateNodes(str:string; path:string = ''; nodeparent:TTreeNode = nil);
var li:TTreeNode;
    s,s2, section:string;
    p:integer;
begin
...
if p > 0 then
   ...
   if li = nil then
      begin
      li:= KeyTree.Items.AddChild(nodeparent, s);
   ...
   if li = nil then
      begin
      li:= KeyTree.Items.AddChild(nodeparent, str);
      li.ImageIndex := 0; li.SelectedIndex := 1;
      TDFTreeNode(li).Section := section;
      end;
...
if datatype = 'DateTime' then
   ...
   frm.calendar_time.DateTime := df.ReadDateTime(section, identifier, Now());
...
sdt := '';
// the tag value must corespond with the TDFType enumeration values
case dt of
     0: begin end;  // unknown
     1: begin  // streams
     ...
     2: begin df.WriteStrings(p, edit_addValue.Text, TStringList.Create());  sdt := 'Strings'; end;
     3: begin end;  // not used: ansistring
     4: begin df.WriteString(p, edit_addValue.Text, '');      sdt := 'String'; end;
     5: begin df.WriteBoolean(p, edit_addValue.Text, false);  sdt := 'Boolean'; sdd := sdd+'FALSE'; end;
     6: begin df.WriteInt8(p, edit_addValue.Text, 0);     sdt := 'Int8'; sdd := sdd+'0'; end;
     7: begin df.WriteInt16(p, edit_addValue.Text, 0);     sdt := 'Int16'; sdd := sdd+'0'; end;
     8: begin df.WriteUInt8(p, edit_addValue.Text, 0);     sdt := 'UInt8'; sdd := sdd+'0'; end;
     9: begin df.WriteUInt16(p, edit_addValue.Text, 0);     sdt := 'UInt16'; sdd := sdd+'0'; end;
     10: begin df.WriteInteger(p, edit_addValue.Text, 0);     sdt := 'Int32'; sdd := sdd+'0'; end;
     11: begin df.WriteInt64(p, edit_addValue.Text, 0);       sdt := 'Int64'; sdd := sdd+'0'; end;
     12: begin df.WriteUInt32(p, edit_addValue.Text, 0);      sdt := 'UInt32'; sdd := sdd+'0'; end;
     13: begin df.WriteUInt64(p, edit_addValue.Text, 0);      sdt := 'UInt64'; sdd := sdd+'0'; end;
     14: begin end;  // not used: real48
     15: begin end;  // not used: extended
     16: begin df.WriteSingle(p, edit_addValue.Text, 0);      sdt := 'Single'; sdd := sdd+'0'; end;
     17: begin df.WriteDouble(p, edit_addValue.Text, 0);      sdt := 'Double'; sdd := sdd+'0'; end;
     18: begin df.WriteCurrency(p, edit_addValue.Text, 0);    sdt := 'Currency'; sdd := sdd+'0'; end;
     19: begin df.WriteDateTime(p, edit_addValue.Text, Now());sdt := 'DateTime'; sdd := sdd+DateTimeToStr(Now()); end;
     20: begin df.WriteFont(p, edit_addValue.Text, frmMain.Font);     sdt := 'Font'; end;
end;
